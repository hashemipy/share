...
class Inventory_Sync_Manager {
    ...
    public function transfer_product($site1_product_id, $site2_product_id = null) {
        ...
        // ابتدا دسته‌بندی‌ها و ویژگی‌های مورد نیاز را در سایت ۲ همگام‌سازی کن
        $this->ensure_categories_exist($product1['categories'] ?? []);
        $this->ensure_attributes_exist($product1['attributes'] ?? []);
    ...
    // =====================================================================
    // اطمینان از وجود دسته‌بندی‌ها در سایت ۲
    // =====================================================================

    /**
     * دسته‌بندی‌های مورد نیاز محصول را در سایت ۲ بررسی و در صورت نبود ایجاد می‌کند.
     * سلسله‌مراتب (parent) هم رعایت می‌شود.
     *
     * @param array $categories   آرایه دسته‌بندی‌های محصول از سایت ۱ (هر آیتم دارای name و slug است)
     */
    private function ensure_categories_exist($categories) {
        if (empty($categories) || ! is_array($categories)) {
            return;
        }

        // دریافت دسته‌بندی‌های موجود در سایت ۲ (نقشه slug → id)
        $site2_cats = $this->get_all_site2_categories();
        ...
        foreach ($categories as $cat) {
            ...
            // اگر از قبل وجود دارد، بدون کار ادامه بده
            if (isset($site2_cats[$slug])) {
                continue;
            }

            // دسته‌بندی والد را از سایت ۱ بگیر و ابتدا آن را ایجاد کن
            $parent_site2_id = 0;
            if (! empty($cat['parent'])) {
                $parent_site1 = $this->site1_api->get_category($cat['parent']);
                if (! is_wp_error($parent_site1) && ! empty($parent_site1['name'])) {
                    $parent_slug = $parent_site1['slug'] ?? sanitize_title($parent_site1['name']);
                    if (! isset($site2_cats[$parent_slug])) {
                        $new_parent = $this->site2_api->create_category([
                            'name' => $parent_site1['name'],
                            'slug' => $parent_slug,
                        ]);
                        if (! is_wp_error($new_parent) && ! empty($new_parent['id'])) {
                            $site2_cats[$parent_slug] = $new_parent['id'];
                        }
                    }
                    $parent_site2_id = $site2_cats[$parent_slug] ?? 0;
                }
            }

            // ایجاد دسته‌بندی در سایت ۲
            $new_cat_data = [
                'name'   => $name,
                'slug'   => $slug,
                'parent' => $parent_site2_id,
            ];
            if (! empty($cat['description'])) {
                $new_cat_data['description'] = $cat['description'];
            }
            if (! empty($cat['image']['src'])) {
                $new_cat_data['image'] = ['src' => $cat['image']['src']];
            }

            $created = $this->site2_api->create_category($new_cat_data);
            if (! is_wp_error($created) && ! empty($created['id'])) {
                $site2_cats[$slug] = $created['id'];
            }
        }
    }

    /**
     * دریافت همه دسته‌بندی‌های سایت ۲ با صفحه‌بندی
     * خروجی: نقشه slug => id
     */
    private function get_all_site2_categories() {
        $map  = [];
        $page = 1;

        do {
            $batch = $this->site2_api->get_categories(100, $page);
    ...
    // =====================================================================
    // اطمینان از وجود ویژگی‌ها و مقادیرشان در سایت ۲
    // =====================================================================

    /**
     * ویژگی‌های مورد نیاز محصول را در سایت ۲ بررسی و در صورت نبود ایجاد می‌کند.
     * مقادیر (terms) هر ویژگی هم بررسی و تکمیل می‌شوند.
     *
     * @param array $attributes   آرایه ویژگی‌های محصول از سایت ۱
     */
    private function ensure_attributes_exist($attributes) {
        ...
        // دریافت ویژگی‌های موجود در سایت ۲ (نقشه slug => id)
        $site2_attrs = $this->get_all_site2_attributes();
        ...
        foreach ($attributes as $attr) {
            ...
            $attr_slug = 'pa_' . sanitize_title($name);

            // اگر ویژگی global (id > 0) نیست، نیازی به ایجاد در taxonomy نیست
            // (ویژگی‌های custom محصول با name/option مستقیماً روی محصول ست می‌شوند)
            if (empty($attr['id']) || intval($attr['id']) === 0) {
                continue;
            }

            // بررسی وجود ویژگی در سایت ۲
            $site2_attr_id = $site2_attrs[$attr_slug] ?? null;

            if (! $site2_attr_id) {
                // ایجاد ویژگی در سایت ۲
                $new_attr = $this->site2_api->create_attribute([
                    'name'         => $name,
                    'slug'         => $attr_slug,
                ...
                if (is_wp_error($new_attr) || empty($new_attr['id'])) {
                    continue;
                }

                $site2_attr_id = $new_attr['id'];
                $site2_attrs[$attr_slug] = $site2_attr_id;
            }

            // دریافت مقادیر این ویژگی از سایت ۱
            $site1_terms = $this->get_all_attribute_terms_site1($attr['id']);

            // دریافت مقادیر موجود در سایت ۲ برای این ویژگی (نقشه slug => id)
            $site2_terms = $this->get_all_site2_attribute_terms($site2_attr_id);

            foreach ($site1_terms as $term) {
                $term_slug = $term['slug'] ?? sanitize_title($term['name'] ?? '');
                if (empty($term['name'])) {
                    continue;
                }

                // اگر از قبل وجود دارد، رد شو
                if (isset($site2_terms[$term_slug])) {
                    continue;
                }

                // ایجاد مقدار جدید
                $new_term = $this->site2_api->create_attribute_term($site2_attr_id, [
                    'name' => $term['name'],
                    'slug' => $term_slug,
                ]);

                if (! is_wp_error($new_term) && ! empty($new_term['id'])) {
                    $site2_terms[$term_slug] = $new_term['id'];
                }
            }
        }
    }
    ...
    /**
     * دریافت همه ویژگی‌های سایت ۲
     * خروجی: نقشه slug => id
     */
    private function get_all_site2_attributes() {
        $map   = [];
    ...
    /**
     * دریافت همه مقادیر یک ویژگی از سایت ۱ با صفحه‌بندی
     * خروجی: آرایه‌ای از term objects
     */
    private function get_all_attribute_terms_site1($attribute_id) {
        $all  = [];
        $page = 1;

        do {
            $batch = $this->site1_api->get_attribute_terms($attribute_id, 100, $page);
            if (is_wp_error($batch) || empty($batch) || ! is_array($batch)) {
                break;
            }
    ...
    /**
     * دریافت همه مقادیر یک ویژگی از سایت ۲ با صفحه‌بندی
     * خروجی: نقشه slug => id
     */
    private function get_all_site2_attribute_terms($attribute_id) {
        $map  = [];
        $page = 1;

        do {
            $batch = $this->site2_api->get_attribute_terms($attribute_id, 100, $page);
            if (is_wp_error($batch) || empty($batch) || ! is_array($batch)) {
                break;
            }
